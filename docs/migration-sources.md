...
> **Historical snapshot:** the checks and skill counts below predate the
> Matt Pocock upstream promotion. They document the initial migration baseline,
> not the current deployment contract. See `README.md` and
> `docs/deployment-inventory.md` for the current state.
