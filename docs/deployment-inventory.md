...
Les huit skills portables sont : `api-design`, `deployment`, `grill-with-docs`,
`mvp`, `nvim-config`, `stack-python`, `stack-rust` et `stack-ts`. La parite porte
sur l'arbre complet, y compris les deux fichiers annexes de `grill-with-docs`.
...
| skills Anthropic | submodule + allowlist de 8 | liens dans `~/.claude/skills` | conserver sous `harnesses/claude/upstream`; meme allowlist |
